...
```bash
# 플러그인으로 설치한 경우 (대부분의 사용자)
python3 ~/.claude/plugins/marketplaces/<marketplace>/cc-audit/skills/cc-audit/audit.py --all-projects

# cc-plugins 레포 작업 중인 경우
python3 cc-audit/skills/cc-audit/audit.py --all-projects
```

`<marketplace>`는 `ls ~/.claude/plugins/marketplaces/`로 확인. 옵션: `--json`(머신 리더블), `--window 30d`(기본 30일, 미만 비권장). 단일 프로젝트 분석이 명확히 필요한 경우에만 `--all-projects` 생략.
